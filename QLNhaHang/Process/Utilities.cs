...
namespace QLNhaHang
{
    class Utilities
    {
        ...
        public Utilities()
        {
            string constr = @"Data Source=DESKTOP-424FVAI;Initial Catalog=QLNhaHang;Integrated Security=True";
        ...
        public DataTable timMonAn(string strMatch)
        {
            con.Open();
            DataTable table = new DataTable();
            string sql = "select * from monan where tenmonan like N'%" + strMatch + "%'";
            SqlDataAdapter adap = new SqlDataAdapter(sql, con);
            adap.Fill(table);
            con.Close();
            return table;
